    ...
    return (
        <div className="bg-red-200 p-2 rounded mt-3 shadow-lg">
            ...
            <select id="reinstall_type" name="type" value={selectedType} onChange={e => setSelectedType(e.target.value)}
                    className="mt-1.5 rounded-lg bg-gray-50 border-gray-300 text-gray-700 sm:text-sm p-2 w-full">
                {types.map((type, i) => (
            ...
            <input type="number" id="reinstall_id" name="id" min="1" value={shopId} onChange={e => setShopId(Number(e.target.value))}
                   className="mt-1.5 rounded-lg border-gray-300 text-gray-700 sm:text-sm p-2 w-full bg-gray-50"/><br/>
